<?php
require_once 'Rubbix_Blocks.php';

class Rubbix_BlocksBlock extends Rubbix_Blocks
{
	...
	public function send($previous, $destination, $balance)
	{
		...
		if($pk === false)
			throw new InvalidRubbix_BlocksAccountException('Destination account is not valid');
	...
	public function open($source, $account, $representative)
	{
		...
		if($a_pk === false)
			throw new InvalidRubbix_BlocksAccountException('Open account is not valid');
		if($r_pk === false)
			throw new InvalidRubbix_BlocksAccountException('Representative account is not valid');
	...
	{
		if(!Rubbix_Blocks::keyFromAccount($xrb_account))
			throw new InvalidRubbix_BlocksAccountException();
	...
	public function setSignature($sig)
	{
		...
		if(!Rubbix_Blocks::checkSig($this->hash->toHexString(), $sig, $this->owner_account))
